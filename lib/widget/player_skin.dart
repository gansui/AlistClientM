...
class AlistPlayerSkinState extends State<AlistPlayerSkin> {
  ...
  void initState() {
    ...
    // restore last playback rate
    final savedRate = SpUtil.getDouble(AlistConstant.lastPlaybackRate, defValue: 1.0) ?? 1.0;
    if (savedRate != 1.0) {
      _rate = savedRate;
      _rateStr = "${savedRate}x";
      // apply after player is prepared
    }
        ...
        child: SingleChildScrollView(
          ...
          children: <Widget>[
              ...
              onPressed: () {
                ...
                onMenuTap(element) {
                  if (_rate != element.rate) {
                    ...
                    _player.setRate(_rate);
                    SpUtil.putDouble(AlistConstant.lastPlaybackRate, _rate);
                    setState(() {});
                  }
                  SmartDialog.dismiss();
                }
            ...
            IconButton(
              ...
              onPressed: _locked ? null : _takeScreenshot,
            ),
            IconButton(
              icon: Icon(
                Icons.flip_rounded,
                color: widget.flipped ? Colors.blue : Colors.white,
              ),
              padding: const EdgeInsets.only(left: 4.0, right: 10.0),
              onPressed: _locked ? null : widget.onToggleFlip,
            ),
            //
          ]),
        ),
  ...
  Future<void> _takeScreenshot() async {
    if (Platform.isAndroid && !(await _checkStoragePermission())) return;
    try {
      final tmpDir = await getTemporaryDirectory();
      final savePath = '${tmpDir.path}/screenshot_${DateTime.now().millisecondsSinceEpoch}.jpg';
      _player.setOnSnapShot((path, playerId) async {
        try {
          await ImageGallerySaver.saveFile(path);
          await File(path).delete();
          if (mounted) SmartDialog.showToast("截图已保存到相册");
        } catch (e) {
          if (mounted) SmartDialog.showToast("保存失败: $e");
        }
      });
      await _player.snapshot(savePath);
    } catch (e) {
  ...
  Future<bool> _checkStoragePermission() async {
    if (await AlistPlugin.isScopedStorage()) return true;
  ...
  GestureTapCallback? _onDoubleTap() {
    return _locked
        ...
        : () {
            ...
            final isRightSide = _lastTapX > _screenWidth / 2;
            final seekDelta = isRightSide ? 10000 : -10000;
            int newPos = _currentPos.inMilliseconds + seekDelta;
            newPos = newPos.clamp(0, _duration.inMilliseconds);
            _player.seekTo(newPos, FlutterAvpdef.INACCURATE);
            setState(() {
              _currentPos = Duration(milliseconds: newPos);
            });
